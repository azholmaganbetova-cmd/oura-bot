...
async def cmd_briefing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    sleep_prev  = sleep_y.get("score", "?")

    workout   = get_workout(today)
    days_left = (MARATHON_DATE - today).days
    race10    = date(2026, 5, 17)
    days_race = (race10 - today).days if today <= race10 else None

    context = (
        f"Дата: {today.strftime('%d.%m.%Y')}\n"
        f"Oura сегодня — Sleep: {sleep_score}/100, Readiness: {ready_score}/100\n"
        f"Oura вчера — Sleep: {sleep_prev}/100\n"
        f"Тренировка по плану: {workout['type']}, {workout['km']} км. {workout['description']}\n"
        f"До марафона (27 сент 2026): {days_left} дней\n"
    )
    ...
    response = claude.messages.create(
        ...
        messages=[{
            ...
            "content": (
                f"{context}\n\n"
                "Составь утреннюю сводку без заголовков и символов markdown:\n"
    ...
    def se(s):
        if s == "?": return "❓"
        return "🟢" if int(s) >= 85 else "🟡" if int(s) >= 70 else "🔴"

    race_emoji = "⚡" if "СТАРТ" in workout["type"] or "МАРАФОН" in workout["type"] else "🌅"
    countdown  = f"До старта: {days_race} дн  |  " if days_race and days_race > 0 else ""

    msg = (
        f"{race_emoji} {today.strftime('%d.%m.%Y')}\n\n"
        f"{se(sleep_score)} Sleep {sleep_score}  |  {se(ready_score)} Ready {ready_score}\n"
        f"🏃 {workout['type']}"
        + (f" · {workout['km']} км" if workout["km"] > 0 else "")
        + f"\n📅 {countdown}До марафона: {days_left} дн\n\n"
        + briefing
    )
